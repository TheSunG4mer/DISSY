...
bob_pk, bob_sk = generate_DH_pubkey(p,G)
alice_pubkey = int(input(f"Alice pubkey:"),16)

print(f"Bob pubkey: {hex(bob_pk)[2:]}")
ss = compute_DH_shared_secret(p,alice_pubkey,bob_sk)

# With a share dsecret, they can securely exchange encrypted messages
alice1_enc = input("alice_to_bob_1:")
alice1_dec = decrypt_message(ss,alice1_enc)

if not alice1_dec == alice1:
    print("That doesn't sound like you Alice! We've been compromised???")
    print("I'm out of here!")
    exit()

bob1_enc = encrypt_message(ss,bob1)
print(f'bob_to_alice_1: {bob1_enc}')

alice2_enc = input("alice_to_bob_2:")
alice2_dec = decrypt_message(ss,alice2_enc)

if not alice2_dec == alice2:
    print("That doesn't sound like you Alice! We've been compromised???")
    print("I'm out of here!")
    exit()

bob2_enc = encrypt_message(ss,bob2)
print(f'bob_to_alice_2: {bob2_enc}')
